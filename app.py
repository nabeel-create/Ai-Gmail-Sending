# ================================================
# 📧 AI Gmail Sender – Gmail Theme (Red & White) + AI Email Generator
# Author: Nabeel + OpenRouter AI
# ================================================
...
for key in ["logged_in", "sender_email", "sender_password", "show_welcome", 
            "openrouter_key", "generated_body", "last_subject", "email_style"]:
    if key not in st.session_state:
        st.session_state[key] = "" if "key" not in ["logged_in", "show_welcome"] else False
...
def help_menu():
    with st.expander("⋮ How to use Gmail Login (App Password / 2FA)"):
        ...
        **Notes:** Normal Gmail password will not work if 2FA is enabled.
        """)

# ------------------------
# OPENROUTER AI GENERATOR
# ------------------------
def generate_ai_email(subject, style):
    if not st.session_state.openrouter_key:
        return "Set your OpenRouter API key first!"
    try:
        client = OpenAI(base_url="https://openrouter.ai/api/v1", api_key=st.session_state.openrouter_key)
        prompt = f"Write a {style} email based on this subject: '{subject}'. Polite, professional, ready to send."
...
def login_page():
    ...
    if st.button("Login"):
        if not email or not password:
            st.warning("Enter both Email & App Password")
        else:
            try:
...
def email_sender_page():
    if st.session_state.show_welcome:
        st.markdown("<div class='welcome-popup'>🎉 Welcome to AI Gmail Sender!</div>", unsafe_allow_html=True)
    ...
    st.sidebar.text_input("OpenRouter API Key", type="password", value=st.session_state.openrouter_key, key="openrouter_key_input")
    help_menu()
    ...
    style = st.selectbox("Select Email Style", ["Professional","Formal","Informal","Friendly","Casual"])

    # Auto-generate AI email body
    if subject and (st.session_state.last_subject != subject or not st.session_state.generated_body):
        st.session_state.generated_body = generate_ai_email(subject, style)
    ...
    if st.button("🚀 Send Emails"):
        if contacts is None:
            ...
            st.warning("Subject required to auto-generate email body!")
        else:
            logs=[]
